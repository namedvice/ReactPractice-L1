...
function KnowledgePanel() {
  ...
  const totalPages: number = Math.ceil(
    (numberOfCats + numberOfDogs) / (currentCatsPerPage + currentDogsPerPage),
  );
  ...
  const renderKnowledgeBlock = async () => {
    ...
    if ((currentPage - 1) * currentCatsPerPage <= numberOfCats) {
      for (
        let i = (currentPage - 1) * CATS_PER_PAGE;
        i < (currentPage - 1) * CATS_PER_PAGE + CATS_PER_PAGE;
        i++
      ) {
        if (i >= numberOfCats) {
    ...
    if ((currentPage - 1) * currentDogsPerPage <= numberOfDogs) {
      ...
      for (
        let i = (currentPage - 1) * currentDogsPerPage;
        i < (currentPage - 1) * currentDogsPerPage + currentDogsPerPage;
        i++
      ) {
        if (i >= numberOfDogs) {
  ...
  useEffect(() => {
    ...
    if (currentDogsPerPage > currentCatsPerPage) {
      document.getElementById("knowledgePanel")!.style.gridTemplateColumns =
        `repeat(${currentDogsPerPage},minmax(13em, 15em))`;
      document.getElementById("knowledgePanel")!.style.gridTemplateColumns =
        `repeat(${currentDogsPerPage},minmax(13em, 15em))`;
    } else {
      document.getElementById("knowledgePanel")!.style.gridTemplateColumns =
        `repeat(${currentCatsPerPage},minmax(13em, 15em))`;
      document.getElementById("knowledgePanel")!.style.gridTemplateColumns =
        `repeat(${currentCatsPerPage},minmax(13em, 15em))`;
    }
  }, [currentPage]);
